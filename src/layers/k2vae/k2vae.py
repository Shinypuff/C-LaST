...
from .koopman import KPLayer, KPLayerApprox, MLP
from .kalman import KalmanFilter
from .auxiliarynet import Transformer as AuxiliaryNet
...
class k2VAE(nn.Module):
    def __init__(self, config):
        ...
        self.koopman = (
            KPLayerApprox(config.dynamic_dim, config.init_koopman)
            if self.multistep
            else KPLayer(config.dynamic_dim, config.init_koopman)
        )
    ...
    def sample(self, x, num_samples):
        ...
        for id, dist in enumerate(dist_list):
            sample_num = (
                k if id < self.config.sample_schedule - 1 else num_samples - cnt
            )
