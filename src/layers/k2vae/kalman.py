...
class KalmanFilter(nn.Module):
    ...
    def one_step(self, x_t_prev, u_t, y_t, P_t):
        ...
        # 1. Prediction step
        x_t_pred = torch.matmul(x_t_prev, self.F.T) + torch.matmul(
            u_t, self.B.T
        )  # [B, state_dim]
        P_t_pred = (
            torch.matmul(self.F, torch.matmul(P_t, self.F.T)) + Q
        )  # [B, state_dim, state_dim]

        # 2. Update step
        H_P_Ht = torch.matmul(
            self.H, torch.matmul(P_t_pred, self.H.T)
        )  # [B, state_dim, state_dim]
        S = H_P_Ht + R  # [B, state_dim, state_dim]
        S_inv = torch.linalg.inv(S)  # Inverse of S, [B, state_dim, state_dim]
        K_t = torch.matmul(
            P_t_pred, torch.matmul(self.H.T, S_inv)
        )  # Kalman gain, [B, state_dim, state_dim]

        # Measurement update
        y_pred = torch.matmul(
            x_t_pred, self.H.T
        )  # Predicted measurement, [B, state_dim]
        x_t = x_t_pred + torch.matmul((y_t - y_pred).unsqueeze(1), K_t).squeeze(
            1
        )  # Updated state [B, state_dim]

        # Joseph stabilized form for P_t
        I = (
            torch.eye(self.state_dim, device=x_t.device)
            .unsqueeze(0)
        ...
        P_t = torch.matmul(
            I_minus_K_H, torch.matmul(P_t_pred, I_minus_K_H.transpose(1, 2))
        ) + torch.matmul(
            K_t, torch.matmul(R, K_t.transpose(1, 2))
    ...
    def keep_positive_definite(self, tensor):
        ...
        fixed_tensor = (
            eigvecs @ torch.diag_embed(eigvals_clamped) @ eigvecs.transpose(-1, -2)
        )
    ...
    def forward(self, x, U_t, Y_t):
        ...
        P_t = (
            torch.eye(self.state_dim, device=x.device)
            .unsqueeze(0)
            .repeat(batch_size, 1, 1)
        )  # Initial covariance [B, state_dim, state_dim]
        ...
        covariance = torch.stack(
            covariance, dim=1
        )  # [B, pred_len, state_dim, state_dim]

        # Create a multivariate normal distribution for the predictions
        try:
            dist = MultivariateNormal(
                loc=torch.zeros_like(prediction).to(x.device),
