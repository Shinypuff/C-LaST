...
class LaSTForecaster(BaseForecasting):
    ...
    def training_step(self, batch, batch_idx, *args, **kwargs):
        ...
        para_mode = batch_idx % 2
        self._set_parameter_mode(para_mode)
        
        if para_mode == 0:
            loss = self.criterion(mean, tgt) - elbo - mlbo + mubo
        else:
            loss = mubo - mlbo

        self.log("train_last_loss", loss, on_step=False, on_epoch=True, prog_bar=True)
        return loss
