...
class GLaSTForecaster(BaseForecasting):
    def __init__(self, input_len=33, output_len=33, var_num=1, hidden_dim=64, dropout=0.1, lr=1e-3, **base_kwargs):
        ...
        # FIX: fusion_net should map from seq_len to pred_len
        print(f"Fusion net: {self.seq_len} -> {hidden_dim} -> {self.pred_len}")
        self.fusion_net = MLP(self.seq_len, hidden_dim, self.pred_len)

        print(f"Mean net: {hidden_dim * 2} -> {hidden_dim} -> {self.target_dim}")
        self.mean_net = MLP(hidden_dim * 2, hidden_dim, self.target_dim)
        
        print(f"Scale net: {hidden_dim * 2} -> {hidden_dim} -> {self.target_dim}")
    ...
    def forward(self, dt, x):
        print(f"=== GLaSTForecaster.forward() ===")
        print(f"x shape: {x.shape}")  
        print(f"dt shape: {dt.shape}") 
        L = x.shape[1]

        dt_simple = dt[:, :L, 0:1]  # [512, 24, 1]
        ...
        z_s, z_t = self.LaSTLayer(x_his)
        print(f"z_s shape: {z_s.shape}, z_t shape: {z_t.shape}")
        ...
        z = self.fusion_net(z)  # Should map [512, 128, 24] -> [512, 128, 24]
