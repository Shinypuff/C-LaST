...
class CLaSTForecaster(BaseForecasting):
    ...
    def sample(self, dt, x, num_samples):
        ...
        z_s, z_t, x_s, x_t, _ = self.LaSTLayer(x_his) # Z: B x T x hid_dim
    ...
    def forward(self, dt, x, num_samples):
        return self.sample(dt, x, num_samples)
    ...
    def training_step(self, batch, *args, **kwargs):
        ...
        z_s, z_t, x_s, x_t, elbo = self.LaSTLayer(x_his)  # Z: B x T x hid_dim
        z_s2, z_t2, _, _, elbo2 = self.LaSTLayer(x_his)
        ...
        loss = F.gaussian_nll_loss(mean, y, scale) - (elbo + elbo2) + (loss_s + loss_t)
