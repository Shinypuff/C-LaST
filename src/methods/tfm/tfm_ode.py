...
class TrajectoryFlowMatchingODE(BaseForecasting):
    ...
    def __init__(
        ...
        **base_kwargs,
        ...
        input_dim = (history + 1) * (1 + self.target_dim) + self.target_dim + 1
        self.mean_mlp = MLP(
            input_dim,
            hidden_dim,
            self.target_dim,
    ...
    def calc_loss(self, dt: Tensor, x: Tensor, y: Tensor):
        z = torch.cat([x, y], dim=1)
        all_feats = torch.cat([dt[:, 1:, None], z[:, :-1]], dim=-1)

        # Shift dts 1 into the future
        time = torch.cumsum(dt, dim=1)
    ...
    def forward(self, dt: Tensor, x: Tensor):
        ...
        y_history = x[:, -(H + 1) :].flip(1)
        time = torch.cumsum(dt, dim=1)

        # mean + scale = dual variable
        dual_y0 = torch.cat([x[:, -1], x.new_zeros(B, D)], dim=-1)
        ...
        for k in range(L, L + T):
            ...
            dt_history = dt[:, k - H : k + 1].flip(1)
            h = torch.cat([dt_history.unsqueeze(-1), y_history], dim=-1).flatten(1, -1)
